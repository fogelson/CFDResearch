...
UniformAdvector::UniformAdvector(int n, double deltaX, double deltaY, Grid * grid){
	this->n = n;
	this->deltaX = deltaX;
	this->deltaY = deltaY;
	this->grid = grid;
 ...
 */
void UniformAdvector::advectFlat(double deltaT, const Array<double,3> & U, Array<double,2> & f){
...
double UniformAdvector::limiter(double theta){
	return 0.0; // Upwind
...
Array<double,2> UniformAdvector::limiter(Array<double,2> theta){
...
Array<double,4> UniformAdvector::make4DArray(){
	Array<double,4> out(Range(0,n-1),Range(0,n-1),grid->xRange,grid->yRange);
	out = 0;
	return out;
}

CellDoubleArray UniformAdvector::copyToSlice(Array<double,4> & f, int i, int j){
...
void UniformAdvector::copyFromSlice(CellDoubleArray & fij, Array<double,4> & f, int i, int j){
	for(int q1 = grid->iMin; q1 <= grid->iMax; q1++){
		for(int q2 = grid->jMin; q2 <= grid->jMax; q2++){
...
void UniformAdvector::advectFromFlat(double deltaT, const Array<double,3> & U, Array<double,4> & f){
	setEdgeVelocities(U,u,v);
...
void UniformAdvector::advect(double deltaT, const Array<double,3> & U, Array<double,4> & f){
	int iM, iP, jM, jP;
 ...
 */
void UniformAdvector::setEdgeVelocities(const Array<double,3> & U, Array<double,2> & u, Array<double,2> & v){
	int iM, jM;
	for(int i = 0; i < n; i++){
		iM = (i + n - 1) % n;
		for(int j = 0; j < n; j++){
