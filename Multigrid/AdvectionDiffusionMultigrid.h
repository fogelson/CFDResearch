/*
 * AdvectionDiffusionMultigrid.h
 *
 *  Created on: Feb 14, 2011
 *      Author: fogelson
 */

#ifndef ADVECTIONDIFFUSIONMULTIGRID_H_
#define ADVECTIONDIFFUSIONMULTIGRID_H_
...
namespace CFD{
	...
	namespace Multigrid{
		 ...
		 * u.
		 */
		GridScalar DiscreteOperator(GridScalar u, Circle circ, double (*boundaryFunction)(double,double)){
			double h = circ.getH();
		...
		GridScalar PoissonVCycle(GridScalar u0, GridScalar f, Circle fineCircle, double (*boundary)(double,double), int v1, int v2){
			...
			u = GSRB(u0, f, (*boundary), fineCircle, v1);
			...
			if(coarseCircle.interiorPoints() < 5){
				eC = GSRB(eC, rC, (*zeroBoundary), coarseCircle, 3);
			}
			else{
			...
			uCorrected = u + eF;
			u = GSRB(uCorrected, f, (*boundary), fineCircle, v2);
			return u;
		}
	}
}

#endif /* ADVECTIONDIFFUSIONMULTIGRID_H_ */
